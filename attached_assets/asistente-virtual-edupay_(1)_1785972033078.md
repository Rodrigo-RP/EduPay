...
## 9. Prevención de catálogo desactualizado

El riesgo más probable de este módulo no es un bug de código, es que el catálogo de intenciones se quede atrás conforme el producto crece. Se previene con cuatro mecanismos, no con disciplina personal de acordarse:

### 9.1 Una sola fuente de verdad para rutas

En lugar de mantener `assistant-knowledge.ts` como un catálogo aislado, todas las rutas del panel administrativo se registran en un único archivo `route-registry.ts` que ya incluye, además del path, una etiqueta y palabras clave asociadas:

```typescript
// packages/shared/route-registry.ts
...
export const APP_ROUTES: AppRoute[] = [
  { path: '/admin/excepciones', label: 'Bandeja de excepciones', keywords: ['excepciones', 'pendientes', 'revisar pagos'] },
  { path: '/admin/importar', label: 'Importación masiva', keywords: ['importar', 'excel', 'carga masiva'] },
];
```

Tanto el menú real de navegación como `matchIntent` consumen este mismo registro. Cuando se agrega una pantalla nueva, se agrega una sola vez, en un solo lugar, y el asistente queda actualizado como efecto automático, no como paso adicional que alguien tiene que acordarse de hacer.

### 9.2 Verificación automática en CI

El pipeline de CI que ya existe para el proyecto corre un script que compara las rutas reales de la aplicación (las que Next.js expone) contra `APP_ROUTES`. Si existe una pantalla sin entrada en el registro, la build falla con un mensaje claro: "La ruta /admin/nueva-pantalla no tiene entrada en route-registry.ts". Esto convierte el olvido humano en un error de compilación detectado antes de llegar a producción, no en un defecto silencioso que descubre el administrador semanas después.

### 9.3 Nunca responder en silencio

Si `matchIntent` no encuentra ninguna coincidencia, el asistente nunca se queda mudo. Responde siempre con una salida de emergencia: una lista corta de las categorías disponibles y el buscador universal como alternativa. Así, incluso si el catálogo está incompleto, el peor caso es "no entendí, pero aquí tienes opciones", nunca silencio total, que es lo que de verdad rompe la confianza.

### 9.4 El catálogo se autocorrige con uso real

Cada mensaje sin coincidencia queda registrado con fecha y contenido exacto. Una revisión periódica de ese registro (semanal o quincenal mientras el producto es nuevo) muestra exactamente qué preguntas está haciendo la gente y que el catálogo no cubre todavía. Esto convierte el mantenimiento de "acordarse de actualizar" a "revisar un reporte concreto de vacíos reales", que es mucho más fácil de sostener en el tiempo.

## 10. Criterio de aprobación (Definition of Done)

Ningún incremento de este módulo se considera terminado si no cumple: el `matchIntent` resuelve correctamente los casos del catálogo de prueba, cada smoke-test corre exclusivamente contra datos sintéticos o de infraestructura, cada corrección automática aplicada queda en `AuditLog`, ningún camino de código permite que el asistente modifique un registro financiero sin confirmación humana explícita, toda ruta nueva del panel administrativo pasa por `route-registry.ts` y la verificación de CI de la sección 9.2 está activa y en verde.
