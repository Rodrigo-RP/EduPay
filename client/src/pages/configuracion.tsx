            ...
            <TabsContent value="conceptos">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Administra los conceptos de pago adicionales específicos de tu institución
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-6">
                    ...
                    <div className="flex gap-4 p-4 border rounded-lg bg-blue-50">
                      <div className="flex-1">
                        ...
                        <Input
                          placeholder="Ej: Papelería, Transporte, Eventos..."
                          value={nuevoConcepto}
                        ...
                        <Button 
                          onClick={() => {
                            if (nuevoConcepto.trim()) {
                              const newId = Math.max(...conceptosPersonalizados.map(c => c.id)) + 1;
                              setConceptosPersonalizados([...conceptosPersonalizados, {
                                id: newId,
                              ...
                              toast({
                                ...
                                description: `El concepto "${nuevoConcepto.trim()}" se ha añadido correctamente`,
                              });
                            }
                          }}
                    ...
                    <div className="space-y-3">
                      <h3 className="text-lg font-semibold text-slate-900">Conceptos Configurados</h3>
                      {conceptosPersonalizados.map((concepto) => (
                        <div key={concepto.id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-slate-50">
                          <div className="flex items-center space-x-4">
                            <div className={`w-3 h-3 rounded-full ${concepto.activo ? 'bg-green-500' : 'bg-red-500'}`}></div>
                            <div>
                              <div className="flex items-center gap-2">
                                <h4 className="font-medium text-slate-900">{concepto.nombre}</h4>
                              ...
                              <p className="text-sm text-slate-600">
                                {concepto.activo ? 'Disponible para asignar a estudiantes' : 'No disponible en nuevos cobros'}
                              </p>
                            </div>
                          </div>
                          <div className="flex items-center space-x-2">
                            ...
                            <Button
                              ...
                              size="sm"
                              onClick={() => {
                                setConceptosPersonalizados(conceptosPersonalizados.map(c => 
                                  c.id === concepto.id ? {...c, activo: !c.activo} : c
                                ));
                                toast({
                                  title: `Concepto ${concepto.activo ? 'desactivado' : 'activado'}`,
                                  description: `${concepto.nombre} ha sido ${concepto.activo ? 'desactivado' : 'activado'}`,
                                });
                              }}
                              title={concepto.activo ? 'Desactivar concepto' : 'Activar concepto'}
                            >
                              {concepto.activo ? (
                            ...
                            <Button
                              ...
                              size="sm"
                              onClick={() => {
                                setEditandoConcepto({id: concepto.id, nombre: concepto.nombre});
                                setNuevoConcepto(concepto.nombre);
                                setCategoriaSeleccionada(concepto.categoria);
                              }}
                            ...
                            <Button
                              ...
                              size="sm"
                              onClick={() => {
                                setConceptosPersonalizados(conceptosPersonalizados.filter(c => c.id !== concepto.id));
                                toast({
                                  title: "Concepto eliminado",
                                  description: `${concepto.nombre} ha sido eliminado del sistema`,
                                  variant: "destructive"
                                });
                              }}
                      ...
                      {editandoConcepto && (
                        <div className="flex gap-4 p-4 border rounded-lg bg-yellow-50">
                          <div className="flex-1">
                            <Label>Editando: {editandoConcepto.nombre}</Label>
                            <Input
                              placeholder="Nuevo nombre del concepto"
                              value={nuevoConcepto}
                              onChange={(e) => setNuevoConcepto(e.target.value)}
                            />
                          </div>
                          ...
                          <div className="flex items-end gap-2">
                            <Button 
                              onClick={() => {
                                if (nuevoConcepto.trim()) {
                                  setConceptosPersonalizados(conceptosPersonalizados.map(c => 
                                    c.id === editandoConcepto.id 
                                      ? {...c, nombre: nuevoConcepto.trim(), categoria: categoriaSeleccionada}
                                      : c
                                  ));
                                  toast({
                    ...
                    <div className="p-4 bg-blue-50 rounded-lg">
                      ...
                      <ul className="text-sm text-blue-800 space-y-1">
                        <li>• Los conceptos personalizados aparecerán en los reportes del sistema</li>
                        <li>• Los conceptos inactivos no se pueden asignar a nuevos estudiantes</li>
                        <li>• Los cobros existentes no se ven afectados al desactivar un concepto</li>
                        <li>• Puedes crear conceptos específicos para tu institución (Seguro escolar, Actividades deportivas, etc.)</li>
                      </ul>
                    </div>
                  </div>
                </CardContent>
