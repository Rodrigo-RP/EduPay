      ...
      {isLoading ? (
        ...
        <>
          ...
          <Card>
            ...
            <CardContent>
              {(topDeudores.length > 0 ? topDeudores : (Array.from({length: 5}, (_, i) => ({
                nombre_familia: `Familia Demo ${i+1}`,
                estudiante: `Estudiante ${i+1}`,
                adeudo_centavos: (8 - i) * 350000,
                dias_vencido: (8 - i) * 15,
                semaforo: i < 2 ? "rojo" : "amarillo",
              })))).slice(0, 10).map((d: any, i: number) => (
